...
::: {#fig-weather layout="[[1, 1, 1], [1, 1], [1]]" fig-cap="These are the grouped figures."}
```{r}
...
dir.create("data/plots/wind")

plot_wind_rose_ggplot(duckdb_con)

wind_paths <- list.files("data/plots/wind")

img <- readPNG(wind_paths[1])
...
plot_wind_rose_ggplot <- function(con) {
     ...
     plot_data <- data |>
       group_by(speed_bin, month_day, time_only, wind_direction_10m) |>
       summarise(count = n())   
     ...
     # Create and store plots for each day
     for (day in days) {
          day_data <- filter(plot_data, month_day == day)
          day_plot <- ggplot(day_data, aes(x = wind_direction_10m, y = count, fill = speed_bin)) +
            geom_col(width = 15, color = "black", size = 0.1) +  # Wider bars for 45° bins
            coord_polar(start = 2*pi) +
            scale_x_continuous(
              limits = c(0, 360),
              breaks = seq(0, 315, by = 45),  # Breaks at cardinal midpoints
              labels = c('N', 'NE', 'E', 'SE', 'S', 'SW', 'W', 'NW')  # Cardinal labels
            ) +
               scale_fill_paletteer_d('ggsci::deep_orange_material') +
               labs(
                    title = paste("Wind Rose -", day),
                    x = "Wind Direction (°)",
                    y = "",
                    fill = "Wind Speed (m/s)"
               ) +
               facet_wrap(~ time_only, ncol = 8) +
               ggplot_theming(
                  ...
                  #  legend.text = element_text(color = 'gray', size = rel(0.5)),
                   # legend.title = element_text(color = 'white', size = rel(0.7))
                   )

          # Store the plot in the list
          plot_list[[day]] <- day_plot
     }
     
     # Combine all plots into a single object using patchwork
     combined_plot <- wrap_plots(plot_list, ncol = 1)  # Arrange plots in a single column
...
data <- dbExecute(connection = duckb_con, query)
```



```{r}
#| label: "fig-patchedPlots"
#| column: screen-inset
#| echo: false
# Call the function and display the combined plot
combined_plot <- plot_wind_rose_ggplot(duckdb_con)

ggsave("data/plots/wind_rose.png", combined_plot, width = 24, height = 20, dpi = 600)
...
data <- dbGetQuery(duckdb_con, query)

# Bin wind speeds and directions
data <- data |>
     mutate(
          speed_bin = cut(
               wind_speed_10m,
          ...
          # Create 15-degree direction bins and convert to numeric
          direction_bin = cut(
               wind_direction_10m,
               breaks = seq(0, 360, by = 15),
               labels = seq(0, 345, by = 15),
               include.lowest = TRUE
