...
As shown in @tbl-cost-breakdown, heavy snow conditions increase costs by **320%** compared to clear skies, primarily due to:

-   Labor costs rising from \$208 to \$1,359 (+553%)
-   Fuel costs increasing by 40%
-   Equipment wear nearly doubling

$$
\text{Optimal Price} = \bigg(\sum_{}{Costs}\bigg)\times \big( 1 + \text{Profit Margin}\big)
$$ Costs

-   Fuel Cost: **`(distance / base_fuel_efficiency) × fuel_multiplier × fuel_price`**

-   Labor Cost: **`(distance / adjusted_speed) × (base_wage × wage_premium)`**

-   Equipment Cost: **`This is route distance multiplied by the base equipment cost per mile and the wear factor from weather.`**

-   Adjusted Speed: **`base_speed_mph / route_delay_factor`**

-   Tolls: **`base_toll × toll_multiplier`**

$$
\text{Equipment Cost} = \underbrace{\text{Distance}}_\text{miles} \times \underbrace{\text{Base Equipment Cost Rate}}_\text{\$/mile} \times \underbrace{\text{Equipment Wear Factor}}_\text{Weather multiplier}
$$

## Estimating Route Delays (doesn't quite work yet)
